...
def write_playlist(folder_path, playlist_file):
    ...
    for subfolder in subfolders:
        ...
        video_files_in_subfolder = [os.path.join(subfolder_path, f) for f in os.listdir(subfolder_path) if f.endswith(".mp4")]
        video_files_in_subfolder.sort()
        video_files.extend([f"{subfolder}/{os.path.basename(f)}" for f in video_files_in_subfolder])
    ...
    for i, filename in enumerate(video_files):
        ...
        if isfile(file_path) and file_path.endswith(".mp4"):
            track = SubElement(tracklist, "track")
            SubElement(track, "location").text = f"file:///{file_path}"
            SubElement(track, "title").text = os.path.splitext(filename)[-2]
